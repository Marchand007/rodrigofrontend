...
router.get('/:id/:user', (req, res, next) => {
    ...
    if (user != req.user.courrielUtilisateur) {
        return next(new HttpError(403, `Vous devez être le même utilisateur pour voir le commentaire`));
    }

    commentQueries.getUserCommentByRecetteId(recetteId, user).then(commentaire => {
        if (commentaire) {
            ...
            return next(new HttpError(404, `Commentaire de ${user} pour la recette ${recetteId} introuvable`));
        }
    }).catch(err => {
        return next(err);
    });
